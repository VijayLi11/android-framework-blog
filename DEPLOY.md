...
- **置顶文章**：编辑器勾选「📌 置顶这篇文章」，或手动在 `js/posts.js` 的文章对象里加 `pinned: true`，置顶文章显示在首页最前。
- **导读分类**：双击 `write.bat` 打开编辑器，拉到底部「📖 导读分类管理」面板——可新增/删除分类、编辑名称和简介、↑↓ 调整顺序、勾选文章加入分类，改完点「💾 保存到 posts.js」。也可手动编辑 `js/posts.js` 中的 `guideCategories` 数组。
- **导读页就地编辑**（推荐）：本地直接打开 `guide.html`，点右上角「✏️ 编辑导读」，可就地修改页面标题、介绍语、各分类大标题和内容概括，点「💾 保存」直接写回 `posts.js`（与编辑器共用同一次文件授权）。编辑入口仅在本地（file:// / localhost）显示，线上访客不可见。页面标题和介绍语数据存在 `js/posts.js` 的 `guidePageConfig` 中。内容概括与介绍语支持 Markdown（链接、加粗等），编辑模式下点「🖼️ 插入图片」可将图片以 Base64 内嵌（建议单张 < 500KB），下方有实时预览。
- **关于页就地编辑**：本地直接打开 `about.html`，点「✏️ 编辑关于页」，可就地修改头像字母、名字、职位、GitHub/邮箱链接、专注领域卡片、技术栈标签（含等级）、经历时间线（支持增删/↑↓ 排序），点「💾 保存」写回 `posts.js` 的 `aboutConfig`（共用同一次文件授权）。编辑入口仅本地可见。

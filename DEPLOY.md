...
- **导读页就地编辑**（推荐）：本地直接打开 `guide.html`，点右上角「✏️ 编辑导读」，可就地修改页面标题、介绍语、各分类大标题和内容概括，点「💾 保存」直接写回 `posts.js`（与编辑器共用同一次文件授权）。编辑入口仅在本地（file:// / localhost）显示，线上访客不可见。页面标题和介绍语数据存在 `js/posts.js` 的 `guidePageConfig` 中。
